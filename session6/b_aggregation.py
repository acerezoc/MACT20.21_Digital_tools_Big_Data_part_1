...
# A simple way to group data based on the columns
count_per_municipality = address.groupby(['localite'], as_index=False).count()
print(count_per_municipality[['localite', 'numero']].head())

#Save the aggregated data set into a csv file
count_per_municipality[['localite', 'numero']].to_csv('../data/luxembourg/address_per_municipality.csv')
